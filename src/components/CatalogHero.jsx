...
const REPAIRS = [
  ...
  { icon: BatteryIcon, name: 'Batería' },
  { icon: CameraIcon, name: 'Cámara' },
  ...
  { icon: ButtonIcon, name: 'Botones' },
  {
    ...
    name: 'Otro arreglo',
    message: '¡Hola! Mi iPhone tiene otra falla o no sé bien qué necesita. ¿Me ayudan?',
  },
]
  ...
  return (
    <section className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto pt-6 sm:pt-8 animate-fade-in">
      <div className="rounded-3xl bg-white dark:bg-[#1c1c1e] border border-black/[0.06] dark:border-white/[0.06] px-6 py-7 sm:px-10 sm:py-9">
        ...
        {/* h2, no h1: el <h1> de la página describe el catálogo (ver CatalogIntro).
            Servicio técnico es una sección más dentro de la home. */}
        ...
        <p className="mt-3 text-[15px] leading-relaxed text-[#6e6e73] dark:text-[#86868b] max-w-3xl">
          Elegí la falla y abrí una consulta rápida por WhatsApp sin salir del catálogo.
        </p>
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          {REPAIRS.map(({ icon: Icon, name, message }) => (
            <a
              key={name}
              href={waLink(message || `¡Hola! Quería consultar por la reparación de ${name} de mi iPhone.`)}
              target="_blank"
        ...
        <div className="mt-6 flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1.5 min-h-[44px] text-[15px] font-semibold tracking-tight text-[#0071e3] hover:gap-2.5 transition-all duration-200"
          >
            Diagnosticar falla
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center min-h-[44px] text-[15px] font-medium text-[#6e6e73] dark:text-[#86868b] hover:text-[#1d1d1f] dark:hover:text-white transition-colors duration-200"
          >
            Ver Instagram
          </a>
        </div>
